...
async updateUserRoles(
  ...
  const roles = [
    ...new Set(updateUserRolesDto.roles),
  ];
  ...
  const hasProfessional =
    roles.includes(UserRole.PROFESSIONAL);

  const currentlyProfessional =
  ...
  return this.usersRepository.updateUserRoles(
    id,
    roles,
  );
}
