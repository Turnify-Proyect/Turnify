...
export class ProfessionalsRepository {
  ...
  async associateService(
    ...
    if (!service.isActive) {
      throw new ConflictException('No se puede asociar un servicio inactivo');
    }
    ...
    const professionalService = this.professionalServicesRepository.create({
      professionalId,
      serviceId,
      professional,
      service,
    ...
    return this.professionalServicesRepository.save(professionalService);
  }
